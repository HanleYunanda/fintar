...
public class ReportService {
    ...
    public DashboardSummaryDTO getDashboardSummary() {
        ...
        long totalAppsLastMonth = loanRepository.countApplicationsBetween(startOfLastMonth, endOfLastMonth);
        ...
        BigDecimal totalOutstanding = totalOutstandingVal != null ? BigDecimal.valueOf(totalOutstandingVal)
                : BigDecimal.ZERO;
        ...
        long approvedThisMonth = loanRepository.countApprovedApplicationsBetween(startOfMonth, endOfMonth);
        double approvalRate = totalAppsThisMonth > 0 ? ((double) approvedThisMonth / totalAppsThisMonth) * 100 : 0.0;
    ...
    public List<DisbursementTrendDTO> getDisbursementTrends() {
        // Last 6 months
        LocalDateTime sixMonthsAgo = LocalDateTime.now().minusMonths(6).with(TemporalAdjusters.firstDayOfMonth())
                .with(LocalTime.MIN);
        // End of this month
        LocalDateTime endOfThisMonth = LocalDateTime.now().with(TemporalAdjusters.lastDayOfMonth()).with(LocalTime.MAX);
